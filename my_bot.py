...
# #         system_prompt = """You are Eliza, you are an inbound caller, to handle pizza orders efficiently, from receiving the order to confirming payment and delivery details
# #         Once your order is confirmed, I will call the delivery team to confirm the delievery address and delivery details for you and will send you a 
# #         notification to the delivery team. Customer satifsaction is """
...
#         self.memory = ConversationBufferMemory(memory_key="chat_history", return_messages=True)
#         self.db_path = db_path
#         self.state = "check_intent"
#         self.requested_time = None
#         self.required_seats = 1
        
#         system_prompt = """You are Eliza, you are an inbound caller, to handle pizza orders efficiently, from receiving the order to confirming payment and delivery details. Customer satisfaction is """
...
# async def get_transcript(callback, audio_chunk):
#     transcription_complete = asyncio.Event()
...
#             # Here you would send audio_bytes to the client if needed
...
# List of pre-recorded audio files for waiting messages
audio_files = [
    'wait.mp3',
    'patient.mp3',
    'checking.mp3',
...
class LanguageModelProcessor:
    def __init__(self, db_path="Restaurant.db"):
        ...
        system_prompt = """You are Eliza, an AI assistant for handling pizza orders. You handle order intake, confirmations, and payments professionally."""
    ...
    def process(self, text):
        ...
        try:
            if "reserve" in text.lower():
                requested_time = self.extract_time_from_text(text)
                if requested_time:
                    return self.handle_reservation_request(requested_time)
                else:
                    return "Please specify a time for the reservation."
            else:
                response = self.conversation.invoke({"text": text})
                return response['text']

        except Exception:
            return "I'm having trouble connecting to my language model service. Please try again shortly."

    def extract_time_from_text(self, text):
        """Extracts time from text input and returns a datetime object."""
        match = re.search(r'\b(\d{1,2})(?::(\d{2}))?\s*(AM|PM|am|pm)\b', text)
        if match:
...
class TextToSpeech:
    ...
    def speak(self, text):
        ...
        url = f"https://api.deepgram.com/v1/speak?encoding=linear16&sample_rate=24000"
        headers = {"Authorization": f"Token {self.DG_API_KEY}", "Content-Type": "application/json"}
        payload = {"text": text}
...
class TranscriptCollector:
    ...
    def add_part(self, part):
        """Appends new transcription part."""
        self.transcript += " " + part

    def reset(self):
        """Resets the transcript collector."""
        self.transcript = ""
...
async def get_transcript(callback, audio_chunk):
    """Streams audio and processes transcriptions asynchronously."""
    transcription_complete = asyncio.Event()
    ...
    try:
        ...
        # Stream the audio in chunks instead of all at once
        chunk_size = 1024  # Stream in small chunks
        for i in range(0, len(audio_chunk), chunk_size):
            dg_connection.send(audio_chunk[i:i + chunk_size])
            await asyncio.sleep(0.1)  # Allow streaming instead of blocking
...
class ConversationManager:
    ...
    async def main(self):
        """Handles the full conversation loop asynchronously."""
        def handle_transcription(text):
            """Callback for handling transcribed text."""
            self.transcription_response = text

        while True:
            print("Playing waiting audio...")
            play(AudioSegment.from_file(random.choice(audio_files)))

            print("Listening for input...")
            await get_transcript(handle_transcription, b"\x00" * 32000)  # Placeholder empty audio
